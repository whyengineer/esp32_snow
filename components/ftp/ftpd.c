...
#define FTP_PORT			21
#define FTP_SRV_ROOT		"/sdcard"
#define FTP_MAX_CONNECTION	4
#define FTP_USER			"we"
#define FTP_PASSWORD		"1"
...
int do_list(char* directory, int sockfd)
{
	...
	ESP_LOGI(TAG,"list dir:%s",directory);
	...
	while (1)
	{
		...
		ESP_LOGI(TAG,"line_buffer:%s",line_buffer);
#ifdef WIN32_NATIVE
...
#endif
		{
			if (s.st_mode & S_IFDIR)
				...
				line_length = rt_sprintf(line_buffer, "-rw-r--r-- 1 admin admin %ld Jan 1 2000 %s\r\n", s.st_size, entry->d_name);
...
int ftp_process_request(struct ftp_session* session, char *buf)
{
	...
	else if(str_begin_with(buf, "PASV")==0)
	{
		...
		{
			// ESP_LOGE(TAG,"data socket create failed");
		...
		{
			//ESP_LOGE(TAG,"set data socket option failed");
			rt_sprintf(sbuf, "425 Can't open data connection.\r\n");
			send(session->sockfd, sbuf, strlen(sbuf), 0);
			goto err1;
		}
		...
		{
			// ESP_LOGE(TAG,"data socket bind failed");
			rt_sprintf(sbuf, "425 Can't open data connection.\r\n");
			send(session->sockfd, sbuf, strlen(sbuf), 0);
			goto err1;
		}
		...
		{
			// ESP_LOGE(TAG,"data socket listen failed");
			rt_sprintf(sbuf, "425 Can't open data connection.\r\n");
			send(session->sockfd, sbuf, strlen(sbuf), 0);
			goto err1;
		}
		rt_kprintf("Listening %ld seconds @ port %d\n", tv.tv_sec, session->pasv_port);
...
err1:
	...
	else if (str_begin_with(buf, "STOR")==0)
	{
		...
		rt_kprintf("Waiting %ld seconds for data...\n", tv.tv_sec);
...
void ftpd_start()
{
	...
	ESP_LOGI(TAG,"ftp server start on port:20");
	xTaskCreate(&ftpd_thread_entry, "ftpd_task",4096, NULL, 5, NULL);
